...
OUTPATH = f"/afs/cern.ch/user/l/lburack/work/BDTdev/EMTF-Study-Tools/Validation/Results/AllModes/mode={MODE}"
...
for event in range(evt_tree.GetEntries()):
    ...
    Y = np.append(Y, log2(evt_tree.genPart_pt[0]))
...
with open(OUTPATH + 'training_data_m=' + str(MODE) + '.pkl', 'wb') as file:
    pickle.dump(X, file)
...
X_train, X_test, Y_train, Y_test, W_train, W_test = train_test_split(X, Y, W, test_size=.1, random_state=123)
X_train_2, X_test_2, Y_train_pt, Y_test_pt, W_train_2, W_test_2 = train_test_split(X, Y_pt, W, test_size=.1, random_state=seed)
X_train_2, X_test_2, Y_train_eta, Y_test_eta, W_train_2, W_test_2 = train_test_split(X, Y_eta, W, test_size=.1, random_state=seed)
X_train_3, X_test_3, Y_train_phi, Y_test_phi, W_train_3, W_test_3 = train_test_split(X, Y_phi, W, test_size=.1, random_state=seed)
